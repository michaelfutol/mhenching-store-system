      ...
      {filteredProducts.length > 0 && (
        <ul className="absolute z-20 w-[calc(100%-2*var(--margin-mobile))] left-margin-mobile mt-1 bg-surface-container-lowest border border-surface-variant rounded-xl shadow-lg max-h-60 overflow-y-auto divide-y divide-surface-variant">
          {filteredProducts.map((p) => (
            (() => {
              const imageUrl = getProductImageUrl(p.image_path);
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className="w-10 h-10 rounded bg-surface-variant bg-cover bg-center flex items-center justify-center text-on-surface-variant group-hover:bg-primary-container group-hover:text-on-primary-container transition-colors"
                      style={imageUrl ? { backgroundImage: `url(${imageUrl})` } : undefined}
                    >
                      {!imageUrl && <span className="material-symbols-outlined text-[20px]">inventory_2</span>}
                    </div>
